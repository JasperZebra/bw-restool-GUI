...
class BattWarsExtractor(tk.Tk):
    ...
    def setup_xml_tab(self):
        ...
        browse_button = tk.Button(self.xml_frame, text="Browse", command=lambda: self.browse_xml_file(self.entry_input))
    ...
    def setup_restool_tab(self):
        # Input file entry with Browse button
        tk.Label(self.restool_frame, text="Input .gz file:").grid(row=0, column=0, padx=10, pady=5)
        self.gz_input = tk.Entry(self.restool_frame, width=50)
        self.gz_input.grid(row=0, column=1, padx=5, pady=5)
        browse_button2 = tk.Button(self.restool_frame, text="Browse", command=lambda: self.browse_gz_file(self.gz_input))
        browse_button2.grid(row=0, column=2, padx=5, pady=5)

        # Destination Directory
        tk.Label(self.restool_frame, text="Destination Directory:").grid(row=1, column=0, padx=10, pady=5)
        self.gz_dst = tk.Entry(self.restool_frame, width=50)
        self.gz_dst.grid(row=1, column=1, padx=10, pady=5)
        browse_dst_button = tk.Button(self.restool_frame, text="Browse", command=lambda: self.browse_dir(self.gz_dst, "Destination"))
        browse_dst_button.grid(row=1, column=2, padx=5, pady=5)

        # Unzip button
        self.process_unzip_button = tk.Button(self.restool_frame, text="Unzip .gz File", command=self.process_gz_file)
        self.process_unzip_button.grid(row=3, column=0, columnspan=3, pady=10)
        
        # Progress bar
        # self.restool_progress = ttk.Progressbar(self.restool_frame, mode="indeterminate")
        # self.restool_progress.grid(row=4, column=0, columnspan=3, sticky="ew", padx=10, pady=5)
    ...
    def setup_resource_tab(self):
        ...
        browse_src_button = tk.Button(self.resource_frame, text="Browse", command=lambda: self.browse_dir(self.entry_src, "Source"))
        ...
        browse_dst_button = tk.Button(self.resource_frame, text="Browse", command=lambda: self.browse_dir(self.entry_dst, "Destination"))
        browse_dst_button.grid(row=1, column=2, padx=5, pady=5)
        ...
        browse_json_button = tk.Button(self.resource_frame, text="Browse", command=lambda: self.browse_json_file(self.entry_json))
        ...
        # Progress bar
        self.resource_progress = ttk.Progressbar(self.resource_frame, mode="determinate")
        self.resource_progress.grid(row=4, column=0, columnspan=3, sticky="ew", padx=10, pady=5)

    def _browse_file(self, obj, file_type_name, *file_types):
        filename = filedialog.askopenfilename(
            title=f"Select {file_type_name} File",
    ...
    def browse_xml_file(self, obj):
        self._browse_file(obj, "XML", ("Level XML Files", "*Level.xml"), ("All Files", "*.*"))
        
    def browse_gz_file(self, obj):
        self._browse_file(obj, ".gz", (".gz Files", "*Level.xml.gz"), ("All Files", "*.*"))

    def browse_json_file(self, obj):
        self._browse_file(obj, "JSON", ("JSON Files", "*.json"), ("All Files", "*.*"))

    def browse_dir(self, obj, location):
        dirname = filedialog.askdirectory(title=f"Select {location} Directory")
        if dirname:
            obj.delete(0, tk.END)
            obj.insert(0, dirname)
    ...
    def process_gz_file(self):
        # Run bat with gz
        try:
            restoolfile = os.path.join(os.path.dirname(__file__), "..", "restool.bat")
            # restoolfile = r"C:/Users/sambe/ALL_BW1and2_STUFF/bw-restool-master/restool.py"
            result = subprocess.run([restoolfile, self.gz_input.get()],
                                    capture_output=True,
                                    text=True,
                                    check=True)
        ...
        try:
            ...
            for key, items in data.items():
                for item in items:
                    ...
                    processed_items += 1
                    self.resource_progress["value"] = (processed_items / total_items) * 100
                    self.update_idletasks()

            messagebox.showinfo("Success", "Resource extraction completed successfully!")
            self.resource_progress["value"] = 0
